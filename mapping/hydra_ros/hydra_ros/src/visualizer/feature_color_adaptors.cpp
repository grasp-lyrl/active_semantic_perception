...
NearestFeatureColor::NearestFeatureColor(const Config& config)
    : config(config::checkValid(config)),
      ...
      colormap_(config.colormap) {
        std::string param_name = "/detector_tasks";
        if (!ros::param::get(param_name, task_list_file)) {
        ROS_WARN("Failed to retrieve 'detector_tasks' from the ROS parameter server.");
      }
      }
...
Color NearestFeatureColor::getColor(const DynamicSceneGraph&,
  ...
  const auto& attrs = node.attributes<SemanticNodeAttributes>();
  bool is_structural_object = (attrs.name.find("Wall") != std::string::npos || 
                                 attrs.name.find("Window") != std::string::npos || 
                                 attrs.name.find("Curtain") != std::string::npos || 
                                 attrs.name.find("Blind") != std::string::npos);
  if (is_structural_object) {
    return spark_dsg::Color(255, 0, 0);
  }
  // const auto result = features_->getBestScore(*metric_, attrs.semantic_feature);
  auto iter = std::find(task_list_file.begin(), task_list_file.end(), attrs.name);
  if (iter != task_list_file.end() && node.layer == DsgLayers::OBJECTS) {
    size_t index = std::distance(task_list_file.begin(), iter);
    return colormap_.getColor(index);
  }
  else if (node.layer == DsgLayers::PLACES || node.layer == DsgLayers::ROOMS)
  {
    const auto result = features_->getBestScore(*metric_, attrs.semantic_feature);
    return colormap_.getColor(result.index);
  }
  else{
    return colormap_.getColor(0); // Default color if not found
  }
}
...
std::string NearestFeatureLabel::getLabel(const SceneGraphNode& node) const {
  ...
  // Use the name we assigned directly. Do not recalculate.
