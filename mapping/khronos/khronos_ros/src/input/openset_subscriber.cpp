...
// }
void OpenSetSubscriber::fillInput(const MsgType& msg, hydra::ImageInputPacket& packet) {
  for (const semantic_inference_msgs::DetectedObject& obj : msg.detections)
  {
    hydra::DetectionFeature feature;
    feature.mask = cv_bridge::toCvCopy(obj.mask)->image;
    feature.bbox = {obj.bbox_xyxy[0], obj.bbox_xyxy[1], obj.bbox_xyxy[2], obj.bbox_xyxy[3]};
    feature.confidence = obj.confidence;
    feature.class_name = obj.label;
    packet.features.push_back(std::move(feature));
  }
}
