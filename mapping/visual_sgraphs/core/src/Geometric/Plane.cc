...
namespace ORB_SLAM3
{
    ...
    const std::vector<Plane::Observation> &Plane::getNewObservations() const
    {
        unique_lock<mutex> lock(mMutexFeatures);
        return observations;
    }
    ...
    {
        std::map<KeyFrame *, Plane::Observation> obsMap;
        return obsMap;
    }

    // Currently, I just maintain a vector of obs instead of using cpp mapping
    void Plane::addObservation(Plane::Observation obs)
    {
        unique_lock<mutex> lock(mMutexFeatures);
