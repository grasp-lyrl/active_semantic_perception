...
namespace ORB_SLAM3
{
    ...
    std::vector<std::pair<typename pcl::PointCloud<PointT>::Ptr, Eigen::Vector4d>> Utils::ransacPlaneFitting(
        typename pcl::PointCloud<PointT>::Ptr &cloud, unsigned int threshold)
    {
        std::vector<std::pair<typename pcl::PointCloud<PointT>::Ptr, Eigen::Vector4d>> extractedPlanes;
        SystemParams *sysParams = SystemParams::GetParams();

        for (unsigned int i = 0; i < sysParams->seg.ransac.max_planes && cloud->points.size() > threshold; i++)
        {
            try
            {
                ...
                {
                    // RANSAC failed. Push an invalid plane marker.
                    typename pcl::PointCloud<PointT>::Ptr emptyCloud(new pcl::PointCloud<PointT>);
                    Eigen::Vector4d invalidPlane = Eigen::Vector4d::Zero(); // Invaid plane
                    extractedPlanes.push_back(std::make_pair(emptyCloud, invalidPlane));
                    continue;
                }
                ...
                // Corresponds to mean 50 and standard deviation 0.5
                extractedCloud = Utils::pointcloudOutlierRemoval<PointT>(extractedCloud,
                                                                         sysParams->sem_seg.pointcloud.outlier_removal.mean_threshold,
                                                                         sysParams->sem_seg.pointcloud.outlier_removal.std_threshold/2);
                
                std::vector<pcl::PointIndices> cluster_indices;
                typename pcl::PointCloud<PointT>::Ptr largestCluster(new pcl::PointCloud<PointT>);
                clusterPlaneClouds(extractedCloud, cluster_indices, sysParams->seg.plane_association.cluster_separation.tolerance);
                if (!cluster_indices.empty())
                {
                    pcl::PointIndices::Ptr largest_cluster_indices(new pcl::PointIndices);
                    size_t largest_cluster_size = 0;

                    // Iterate through all found clusters to find the one with the most points
                    ...
                    extract.setInputCloud(extractedCloud);
                    extract.setIndices(largest_cluster_indices);
                    extract.setNegative(false); // Keep the points that are in the indices list
                    extract.filter(*largestCluster);
                }
                else
                {
                    continue;
                ...
                extractedPlanes.push_back(std::make_pair(largestCluster, plane));
    ...
    Utils::ransacPlaneFitting<pcl::PointXYZRGBA, pcl::SACSegmentation>(
        pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &, unsigned int);
    template std::vector<std::pair<pcl::PointCloud<pcl::PointXYZRGBA>::Ptr, Eigen::Vector4d>>
    ...
    ORB_SLAM3::Plane::planeVariant Utils::getPlaneTypeFromClassId(int clsId)
    {
        ...
        {
        case 0:
            return ORB_SLAM3::Plane::planeVariant::CURTAIN;
        case 1:
            return ORB_SLAM3::Plane::planeVariant::BLIND;
        case 2:
    ...
    int Utils::getClassIdFromPlaneType(ORB_SLAM3::Plane::planeVariant planeType)
    {
        switch (planeType)
        {
        case ORB_SLAM3::Plane::planeVariant::CURTAIN:
            return 0;
        case ORB_SLAM3::Plane::planeVariant::BLIND:
            return 1;
                               ...
                               const float threshold)
    {
        ...
        if (sysParams->seg.plane_association.cluster_separation.enabled && planeId == -1 && !checksForCluster.empty())
        {
            for (const auto &check : checksForCluster)
            {
                ...
                double minimum_cloud_dis = calculateMinimumDistance(mPlane->getMapClouds(), givenCloud);
                ...
                // clusterPlaneClouds(aggregatedCloud, clusterIndices, sysParams->seg.plane_association.cluster_separation.tolerance);
                // if (clusterIndices.size() > 1)
                //     continue;
                if (minimum_cloud_dis > sysParams->seg.plane_association.cluster_separation.tolerance)
                {
                    continue;
                }
    ...
    void Utils::clusterPlaneClouds(const pcl::PointCloud<pcl::PointXYZRGBA>::Ptr &cloud, std::vector<pcl::PointIndices> &clusterIndices, double tolerance)
    {
    ...
    /**
     * @brief Calculates the minimum distance between two point clouds.
     *
     * @param cloudA The first point cloud.
     * @param cloudB The second point cloud.
     * @return The minimum distance in meters. Returns a very large number if one cloud is empty.
     */
    double Utils::calculateMinimumDistance(
        const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr& cloudA,
        const pcl::PointCloud<pcl::PointXYZRGBA>::ConstPtr& cloudB)
    {
        ...
        // 1. Create a Kd-tree from the first cloud (cloudA)
        pcl::KdTreeFLANN<pcl::PointXYZRGBA> kdtree;
        kdtree.setInputCloud(cloudA);

        double minDistanceSquared = std::numeric_limits<double>::max();

        // 2. Iterate through each point in the second cloud (cloudB)
        for (const auto& pointFromB : *cloudB)
        {
            // Buffers for the search results (we only need the single nearest neighbor)
            std::vector<int> pointIdxNKNSearch(1);
            std::vector<float> pointNKNSquaredDistance(1);

            // 3. Search for the single nearest neighbor (k=1)
            if (kdtree.nearestKSearch(pointFromB, 1, pointIdxNKNSearch, pointNKNSquaredDistance) > 0)
            {
                // 4. Check if this point's distance is the new minimum.
                //    We compare squared distances to avoid costly sqrt() in the loop.
                if (pointNKNSquaredDistance[0] < minDistanceSquared)
                {
                    minDistanceSquared = pointNKNSquaredDistance[0];
                }
            }
        }

        // 5. Return the actual distance
    ...
    void Utils::reAssociateSemanticPlanes(Atlas *mpAtlas)
    {
        ...
        for (const auto &plane : planes)
        {
            ...
            if (matchedPlaneId != -1)
            {
                ...
                for (const auto &obs : smallPlane->getNewObservations())
                    bigPlane->addObservation(obs);
