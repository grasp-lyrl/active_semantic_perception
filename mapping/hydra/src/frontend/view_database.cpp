...
ViewDatabase::ViewDatabase(const Config& config, const int scene_graph_id)
...
void ViewDatabase::updateAssignments(const DynamicSceneGraph& graph,
  ...
  auto& queue = PipelineQueues::instance(scene_graph_id_).input_features_queue;
  size_t new_views = 0;
  while (!queue.empty()) {
    ++new_views;
  ...
  while (iter != views_.end()) {
    ...
    for (const auto node_id : active_places) {
      ...
      if (view->pointInView(graph.getNode(node_id).attributes().position, view->depth_image)) {
        visible = true;
        break;
      }
    }
