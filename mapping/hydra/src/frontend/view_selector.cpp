...
// FeatureView without depth image (reduce memory usage but cannot check occlusion)
FeatureView::FeatureView(uint64_t timestamp_ns,
                         const Eigen::Isometry3d& sensor_T_world,
                         const FeatureVector& feature,
                         const Sensor* sensor)
...
bool FeatureView::pointInView(const Eigen::Vector3d& point_w,
  ...
  if (depth_image.empty()) {
    // if no depth image, just check frustum
    return sensor_->pointIsInViewFrustum(p_s.cast<float>());
  }
  else {
    // if depth image available, check frustum and occlusion
    return sensor_->pointIsInViewFrustumWithOcclusion(p_s.cast<float>(),depth_image);
  }
}
...
struct BoundaryViewSelector : ViewSelector {
  void selectFeature(const FeatureList& views,
    ...
    for (const auto& view : views) {
      ...
      Eigen::Vector3d p_s;
      if (!view->pointInView(attrs.position, view->depth_image, &p_s)) {
        continue;
      }
...
struct FusionViewSelector : ViewSelector {
  void selectFeature(const FeatureList& views,
    ...
    for (const auto& view : views) {
      ...
      if (!view->pointInView(attrs.position, view->depth_image)) {
        continue;
      }
